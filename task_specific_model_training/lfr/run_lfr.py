...
df_split = pd.read_csv(f'{int_path}/tvt_split_stratified_2dx.csv')
df_pop = df_pop.merge(df_split[['person_id', 'split']], how = 'inner', on = 'person_id')
df_pop['privileged_group'] = 0
df_pop.loc[df_pop['race_concept_id']==8527, 'privileged_group'] = 1
df_pop['is_Male'] = 0
df_pop.loc[df_pop['gender_concept_id']==8507, 'is_Male'] = 1
df_data = df_data.merge(df_pop[['person_id', 'split', 'privileged_group', 'is_Male', 'sz_flag']], how = 'inner', on = 'person_id')

with open(f'{int_path}/10_31_mdcd_2dx_1yrpred_colnames_dict', "rb") as fp:   # Unpickling
    data_columns_dict = pickle.load(fp)
data_columns = data_columns_dict['Z'] + data_columns_dict['W'] + ['is_Male']
label_name = "sz_flag"
prot_attr = "privileged_group"

df_data_train = df_data.loc[df_data['split']=='train']
df_data_val = df_data.loc[df_data['split']=='val']
df_data_test = df_data.loc[df_data['split']=='test']

# standard scale the data
scaler = StandardScaler()
train_data_mat = scaler.fit_transform(df_data_train[data_columns])
df_data_train[data_columns] = train_data_mat
df_data_val[data_columns] = scaler.transform(df_data_val[data_columns])
df_data_test[data_columns] = scaler.transform(df_data_test[data_columns])
...
def save_transformed_results(model, dataset, original_df, filename):
    ...
    cols = ['person_id', 'y_true', 'y_pred_hard', 'y_pred']
