...
# set up dataset
"""
SPECIFY PATHS AND LOAD DATA COLUMNS
"""
px_z_df = pd.read_csv(f'{int_path}/10_31_mdcd_2dx_1yrpred_px_z.csv')
...
raw_data_effects = pd.read_csv('../effect_estimates/effect_estimates/raw_data/estimated_effects_splittrain_featurestop20percent.csv')
...
raw_data_nse = raw_data_effects['Estimated_SE_sn'].mean()

px_z_df = px_z_df[['person_id', 'px_z']].set_index('person_id')
for lmbda in [0.1, 1, 10, 100]: # 0.1, 1, 10, 100
    ...
    loss_regularization_dict = {'regularization_type': 'causal',
                                ...
                                'dict_effects': {'nde': (0, lmbda), 'nie': (0, lmbda), 'nse': (0, lmbda)},
