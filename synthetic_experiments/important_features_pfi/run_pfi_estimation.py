...
import cupy as cp
import sys
from important_feature_pfi_utils import *
...
def run_for_importance(dimw, feature_size_list, results_path, device):
    ...
    ranking_df = get_most_important_features(df, full_W_cols, full_W_cols, 'Y', device=device, binary = True)
    ranking_df.to_csv(f'feature_selection_model_results/feature_importance_largew_ranking_dimw{dimw}_dimz{dimz}.csv')
