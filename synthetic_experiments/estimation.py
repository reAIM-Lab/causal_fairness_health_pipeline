...
def estimate_effects(
        sample, all_df, X, Y, W_cols, Z_cols, Y_binary,
        N, device, clip=0.0, K=5, bootstraps=30, n_mc=1000, 
        sample_kwargs=None, hparam_path=None, grid_search = grid_search_xgb
    ):
    ...
    for n in N:
        ...
        for i in tqdm(range(bootstraps)):
            ...
            for tr, ts in folds.split(all_idxs):
                ...
                y_wvzx1 = fit(df_mu_x1[W_cols + Z_cols], df_mu_x1[Y], device, binary=Y_binary, **hp_dict['y_wvzx1'])
                df_ns_x0['mu2_ne'] = pred(df_ns_x0[W_cols + Z_cols], y_wvzx1, binary=Y_binary)
                df.loc[ts, 'mu2_ne'] = pred(df_ts[W_cols + Z_cols], y_wvzx1, binary=Y_binary)
...
def run_causal_estimates(df, ns, W_cols, Z_cols, dict_dims, device, results_path):
    list_results = []
    list_submodel_performances = []
    dimw = dict_dims['W']
    dimz = dict_dims['Z']

    for n_sample in ns:
        sample_df = df.sample(n=n_sample, random_state = 31)
        results_df, submodel_performances = estimate_effects(
            None, sample_df, 'X', 'Y', 
            W_cols, Z_cols, True,
            N=n_sample, device = device, bootstraps=100, K=5, clip=1e-4, 
            n_mc=n_sample, sample_kwargs={'dims':dict_dims},
            hparam_path=f'{results_path}/hparams_dimw{dimw}_dimz{dimz}.json', grid_search = grid_search_xgb)
        results_df['n'] = n_sample
        submodel_performances['n'] = n_sample
        list_results.append(results_df)
        list_submodel_performances.append(submodel_performances)
    results_df = pd.concat(list_results)
    submodel_performances_df = pd.concat(list_submodel_performances)
    return results_df, submodel_performances_df
