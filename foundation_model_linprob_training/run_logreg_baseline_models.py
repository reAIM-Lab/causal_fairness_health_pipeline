...
# NOTE this can be used for the DA model and the DU model (just remove demo_cols)

# set up dataset
def main(disease, awareness):
    print(f'Running {disease} {awareness}')
    path = f'PATH TO DATA'

    device = torch.device("cpu")
    save_directory = f'PATH TO MODEL'
    ...
    hcu_col = ['hcu']
    data_columns = embedding_cols + hcu_col 
    if awareness == 'da':
        data_columns = data_columns + demo_cols
    ...
    # create dataloaders
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=len(train_dataset), shuffle = True, pin_memory = True, num_workers = 4, persistent_workers = True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=len(train_dataset), shuffle = True, pin_memory = True, num_workers = 4, persistent_workers = True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=len(train_dataset), shuffle = True, pin_memory = True, num_workers = 4, persistent_workers = True)
    ...
    grid_search_all = {'learning_rate': [1.0, 0.1],
                'optimizer':['LBFGS'],
    ...
    for params in permutations_dicts:
        ...
        model_config['params'] = {'input_dim': len(data_columns)}
